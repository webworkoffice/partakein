...
public class SendEnvelopeTask extends Transaction<Void> {
    ...
    private boolean sendDirectMessage(PartakeConnection con, DataIterator<Envelope> it, Envelope envelope) throws DAOException {
        ...
        UserPreference pref = DBService.getFactory().getUserPreferenceAccess().find(con, receiverId);
        if (pref == null) {
        ...
        UserEx user = UserDAOFacade.getUserEx(con, receiverId);
        ...
        try {
            Message message = DBService.getFactory().getDirectMessageAccess().find(con, envelope.getMessageId());
    ...
    private boolean sendTwitterMessage(PartakeConnection con, DataIterator<Envelope> it, Envelope envelope) throws DAOException {
        ...
        UserEx sender = UserDAOFacade.getUserEx(con, senderId);
        if (sender == null) {
    ...
    private void markAsUnauthorizedUser(PartakeConnection con, UserEx user) {
        ITwitterLinkageAccess access = DBService.getFactory().getTwitterLinkageAccess();
