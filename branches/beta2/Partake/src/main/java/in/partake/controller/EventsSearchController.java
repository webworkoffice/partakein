...
public class EventsSearchController extends DeprecatedPartakeActionSupport {
	/** */
	...
	public String search() {
		...
		if (searchTerm == null || category == null || sortOrder == null) {
			try {
				...
				logger.error("DAOException occured.", e);
				return ERROR;
			}
		}
		...
		try {
		    ...
		    logger.debug("any illegal argument was specified.", e); // Intentionally used "debug" instead of "warn".
			addActionError("検索条件のパーズに失敗しました。");
			return INPUT;
		} catch (ParseException e) {
		    logger.debug("Parsing failed.", e);
			addActionError("検索された条件が不正です。見直してください。");
			return INPUT;
		} catch (DAOException e) {
			logger.error("DAOException occured.", e);
			return ERROR;
