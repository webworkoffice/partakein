    ...
    private List<UUID> prepareEvents(int n) throws Exception {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            Event event = new Event(null, "title", "summary", category,
                    ...
                    EVENT_FOREIMAGE_ID, EVENT_BACKIMAGE_ID, isPrivate ? "passcode" : null, false, new ArrayList<EventRelation>(),
                    now, now, -1);

            String eventId = storeEvent(event);

            UUID uuid = UUID.randomUUID();
            EventTicket ticket = new EventTicket(uuid, eventId, "Free", TicketType.FREE_TICKET, 0, null, null, TimeUtil.getCurrentDateTime(), null);
