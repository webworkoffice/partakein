...
<div id="editor-show">
    <div id="editor-content">
        <% if (event.getEditors() != null) { for (User editor : event.getEditors()) { %>
            <div>
                <a href="/users/<%= editor.getId() %>"><img src="<%= h(editor.getProfileImageURL()) %>" height="20" width="20"><%= h(editor.getScreenName()) %></a>
                <input type="hidden" name="editorId" value="<%= h(editor.getId()) %>">
            </div>
        <% } } %>
    </div>
...
<script>
var initialEditors = [
<% if (event.getEditors() != null) { for (int i = 0; i < event.getEditors().size(); ++i) { User editor = event.getEditors().get(i); %>
    <% if (i != 0) { %>,<% } %>
    {
        id: '<%= h(editor.getId()) %>',
        screenName: '<%= h(editor.getScreenName()) %>',
        profileImageURL: '<%= h(editor.getProfileImageURL()) %>'
    }
<% } } %>
];
...
<div class="input-prepend">
    <div class="add-on"><input id="date-end-use-input" type="checkbox" class="edit-input" name="usesEndDate" <%= event.getEndDate() != null ? "checked" : "" %> /></div><input type="text" id="date-end-input" name="endDate" placeholder="YYYY-MM-DD HH:MM" value="<%= event.getEndDate() != null ? TimeUtil.formatForEvent(event.getEndDate()) : TimeUtil.formatForEvent(event.getBeginDate()) %>" />
</div>
<div class="edit-form-buttons">
    <input type="button" value="キャンセル" class="btn edit-cancel-button">
...
<div class="event-map"><a id="address-anchor" href="http://maps.google.co.jp/maps?q=<%= event.getAddress() != null ? h(Util.encodeURIComponent(event.getAddress())) : "" %>">
    <img id="address-img" src="http://maps.google.co.jp/maps/api/staticmap?size=280x200&center=<%= event.getAddress() != null ? h(Util.encodeURIComponent(event.getAddress())) : "" %>&zoom=17&sensor=false" />
</a></div>
...
%>
var relatedEventIds = [
<%= StringUtils.join(relatedEventIds.iterator(), ",") %>
];
